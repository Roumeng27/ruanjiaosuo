...
export default {
	...
	data(){
		return {
			...
			qualificationExaminationMethods:'',
			electronicBidEvaluation:'',// 电子化开评标
			electronicBidEvaluations:'',
			amountId:''
		}
	},
	created(){
	...
	methods:{
		...
		searchList(){
			...
			if(this.role_types == '招标(采购)人'){
				urlStr = "/projectInformationController/selectCommissionedBidding"
			}else{
				urlStr = "/projectInformationController/selfTenderList"
			}
		...
		handle(row){
			...
			window.sessionStorage.removeItem('str_entrust_type');
			this.$router.push({
