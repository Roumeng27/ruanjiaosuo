    ...
    methods:{
                ...
                if(res.data.status == 200){
                    ...
                    this.resultData.forEach(item=>{
                        if(item.startBidTime !=null){
                            item.startBidTime = dayjs(item.startBidTime).format('YYYY-MM-DD');// 上传截止日期
                        }
                        if(item.uploadTime !=null){
                            item.uploadTime = dayjs(item.uploadTime).format('YYYY-MM-DD');// 最新操作日期
                        }
        ...
        getSearchList(totalProjectId){
            ...
            this.$axios({
                ...
                this.assessmentTime = dayjs(res.data.data[0].assessmentTime).format('YYYY-MM-DD');// 开标时间
            }) 
        },
        // 招标项目信息
