...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            agreeSubmit(){
                let object = {
                    bidDocumentId:this.referenceId,
                    state:'办理通过',
                    operationFlow:'招标文件'
                }
                this.$axios({
                ...
                this.$axios({
                    ...
                    if(res.data.status == 200){
                        res.data.data.forEach(item=>{
                            ...
                            this.bidOpeningMethod = item.bidOpeningMethod;//开标方式
                            this.depositAmount = item.depositAmount;//保证金金额
                            this.depositReceive = item.depositReceive;//保证金接收账户
                            this.depositMethod = item.depositMethod;// 保证金允许递交方式
                            this.bidPeriod = item.bidPeriod;// 投标有效期天数
